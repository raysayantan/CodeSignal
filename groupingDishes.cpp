...
std::vector<std::vector<std::string>> groupingDishes(std::vector<std::vector<std::string>> dishes) {
    ...
    int numOfDishes = dishes.size();
    vector<vector<string>> output;

    for(int idx = 0; idx < numOfDishes; idx++){
        int ingredientCounts = dishes[idx].size();

        string dishName = dishes[idx][0];
        set<string> temp;
        for(int k = 1; k < ingredientCounts; k++){
            if(ingredients.find(dishes[idx][k]) != ingredients.end()){
                temp = ingredients[dishes[idx][k]];
                temp.insert(dishName);
                ingredients[dishes[idx][k]] = temp;
            } else {
                temp.insert(dishName);
                ingredients[dishes[idx][k]] = temp;
            }
