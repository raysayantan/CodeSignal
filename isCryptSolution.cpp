...
bool isCryptSolution(std::vector<std::string> crypt, std::vector<std::vector<char>> solution) {
    ...
    for(int i = 0; i < 3; i++){
        ...
        int val = 0;
        for(int j = 0; j < len; j++){
            if(valueMap.find(crypt[i][j]) != valueMap.end()){
                val = valueMap[crypt[i][j]] - '0';
            } else {
                for(int k = 0; k < solLen; k++){
                    if(solution[k][0] == crypt[i][j]){
                        val = solution[k][1] - '0';
                        valueMap[solution[k][0]] = solution[k][1];
                        break;
                    }
                }
            }
            
            if(j == 0 && val == 0 && len > 1) return false;
            if( i == 0){
                word1 = word1*10 + val;
            } else if(i == 1){
                word2 = word2*10 + val;
    ...
    if(word1 + word2 == word3) result = true;
    return result;
}
